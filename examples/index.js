...
var serviceAccount = require("C:/Users/richa/nfc-pcsc/rpaykey.json");

admin.initializeApp({
  credential: admin.credential.cert(serviceAccount),
  databaseURL: "https://rpay-a1711.firebaseio.com"
});
...
function writeNewPost() {
   admin.database().ref('users/' + "test").set({
    username: "Test",
    email: "test"
  });
}
...
nfc.on('reader', async reader => {
	...
	reader.on('card', async card => {
		...
		// Android HCE
			...
			var dateString = new Date().toString('yyyy-MM-dd-HH-mm-ss') ;
			var s = dateString + guid;
			if(data != null)
			{
				console.log('point 1');
				return admin.database().ref('/users/' + data).once('value').then(function(snapshot) {
					console.log('point2');
				var balance = (snapshot.val() && snapshot.val().balance) || 'Anonymous';
				console.log(balance);
				
				console.log(balance);
				if (balance >= transb)
				{	
					var newbalance = balance - transb;
					admin.database().ref('users/' + data + '/balance').set(newbalance);		
					admin.database().ref('users/' + data + '/transactions/' + s ).set({
					amount: transb,
					transDate: dateString,
					transId: guid,
					transInfo: 'Demo transaction'
				 
  });
			}
			else
			{
				console.log('Funds are insufficent for Transaction');
			}
});
		}}
